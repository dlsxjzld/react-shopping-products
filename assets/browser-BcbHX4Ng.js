...
import { A as ALL_CATEGORY, L as LOW_PRICE_SORT_KEY, H as HIGH_PRICE_SORT_KEY } from "./index-B6tuEl6M.js";
...
const addCartItem = http.post(BASE_URL, async ({ request }) => {
  ...
  if (newProduct && typeof newProduct === "object" && "productId" in newProduct && "quantity" in newProduct) {
    ...
    return HttpResponse.json(void 0, { status: 201 });
  }
});
const deleteCartItem = http.delete(`${BASE_URL}/:id`, ({ params }) => {
  const { id } = params;
  const cartItemId2 = Array.isArray(id) ? id[0] : id;
  const convertedCartItemId = Number.parseInt(cartItemId2, 10);
...
const updateCartItemQuantity = http.patch(
  ...
  async ({ params, request }) => {
    ...
    if (!updatedData || typeof updatedData !== "object" || !("quantity" in updatedData)) {
      return HttpResponse.json(
    ...
    return HttpResponse.json(void 0, { status: 200 });
  }
);
...
const productListHandler = http.get(
  ...
  ({ request }) => {
    ...
    const response = {
      ...
      pageable: {
        pageNumber,
        pageSize,
        sort: sortParam ? { empty: false, sorted: true, unsorted: false } : { empty: true, sorted: true, unsorted: true },
        offset: startIndex,
        paged: true,
        unpaged: false
      },
...
const handlers = [productListHandler, ...cartItemHandler];
